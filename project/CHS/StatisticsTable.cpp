...
#endif
{
	...
	{ 
		...
		if(it == m_mapSSNUser.end())
		{
			if(lDiff==1)
			{
				...
				theLog.Put(INF_UK, "GLS_CUInf, new ssn registerd in m_mapSSNUser. SSN=", lSSN);
			}
			if(lDiff == -1) //등록되지 않은 SSN에 대한 사용자 삭제이기 때문에 Error
			{
				theLog.Put(WAR_UK, "GLS_CUWar,SetStatInfo() not exist user. RoomID=", CRoom::RoomID2Str(rid), ", usn=", info.m_lUSN, ", debug_type=", debug_type);
				return FALSE;
			}
		}
		else
		{
			...
			if(iter != usnCount.end()) //list에 USN 있을 경우
			{
				...
				case 1: // 사용자 추가인데 이미 usn이 들어 있다 Error
					...
					theLog.Put(WAR_UK, "GLS_CUWar,SetStatInfo() already exist user. Count++:", usnCount[info.m_lUSN],", RoomID=", CRoom::RoomID2Str(rid), ", usn=", info.m_lUSN, ", usnCountSize= ", usnCount.size(), ", debug_type=", debug_type);
				...
				case -1: // 있는 사용자 제거이므로 
					...
					{
						theLog.Put(WAR_UK, "GLS_CUWar,SetStatInfo(). RemoveReq But Count is not Zero. Count--:", usnCount[info.m_lUSN], ", RoomID=", CRoom::RoomID2Str(rid), ", usn=", info.m_lUSN, ", usnCountSize= ", usnCount.size(), ", debug_type=", debug_type);
						return FALSE;
					}
					break;
				default: // 현재 lDiff는 1,-1 로만 호출한다.
					theLog.Put(WAR_UK, "GLS_CUErr,SetStatInfo(). lDiff=", lDiff, " debug_type=", debug_type);
					break;
				}			
			}
			else //list에 USN이 없는 경우
			{
				...
				case -1: // 없는 usn에 대해 삭제 요청. Error
					usnCount[info.m_lUSN] = -1;
					theLog.Put(ERR_UK, "GLS_CUWar,SetStatInfo() RemoveReq But not exist user. RoomID=", CRoom::RoomID2Str(rid), ", usn=", info.m_lUSN, ", usnCountSize= ", usnCount.size(), ", debug_type=", debug_type);
					return FALSE;
	...
	if (m_TotalStatInfo.m_lTotCount < 0) {
		theLog.Put(ERR_UK, "GLS_CUErr,SetStatInfo() m_TotalStatInfo.m_lTotCount=[", m_TotalStatInfo.m_lTotCount,"]");
	...
	else
	{
		...
		theLog.Put(WAR_UK, "GLS_Stat_Error"_COMMA, sLogTemp.c_str());
	}

	if(it == mStatMap.end())
	{
		mStatMap[lSSN] = statTemp;
		theLog.Put(INF_UK, "GLS_CUInf, new ssn registerd in mStatMap. SSN=", lSSN);
	}
	...
	for (int i = 0; i < 40 ; i++)
		if (debug[i] < 0 )
			theLog.Put(ERR_UK, "GLS_CUErr,SetStatInfo() SSN=",lSSN,", index=",i,", value=[", debug[i],"]");
#endif
...
#endif
{
	...
	if (m_TotalStatInfoPC.m_lTotCount < 0) {
		theLog.Put(ERR_UK, "GLS_CUErr,SetStatInfo() m_TotalStatInfoPC.m_lTotCount=[", m_TotalStatInfoPC.m_lTotCount,"]");
...
void CStatisticsTable::SaveMsgCount()
{
	...
	ForEachElmt(MsgCountMap, mapMsgCountTemp, i, j)
	{
		...
		if (!bRet)
			LOG(INF_UK, "CHS_SaveMsg_Error"_LK, "gDBGW Query Failure: ", nResult, ", Query :", sQuery);
		else
			LOG(DEV_UK, "CHS_SaveMsg_Success"_LK, "gDBGW Query Success: ", sQuery);
	}
}
