...
BOOL CControl::RunEx()
{
	...
	if(!bRet)
	{
		...
		LOG(INF_UK, "NLS_CONTROL, ### LRBHandler Run Failed !!! ###");
		return FALSE;
	}
	LOG(INF_UK, "NLS_CONTROL, ##### LRBHandler Run Success!!! #####");
    
	//theLRBHandler.ReqSvrRegister(NULL);
	::Sleep(1000);
...
static LONG g_iRetryCnt = 0L;
BOOL CControl::RecoverUserListEx()
{
...
BOOL CControl::GetComputerName(string & comp_name)
{
	...
	if(!::GetComputerNameA(buff, &dwLen)) {
		LOG(ERR_UK, "NLS_CONTROL, +++++ fail GetCoumpterName +++++");
		return FALSE;
	}
	comp_name = buff;
	return TRUE;
