...
BOOL Agent::RunAgent()
{
	...
	if (!bRet)
	{
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "Failed to create reconnect timer");
		return FALSE;
	}
	*/
...
STDMETHODIMP_(void) Agent::OnSignal(HSIGNAL hSignal, WPARAM wParam, LPARAM lParam)
{
	...
	{	
		if(wParam == AGENT_POSTMSG)
		{
			...
			{
				theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] fatal error : occur event but empty Queue ");
			}
		}
	}
	/*
	else 
	{
		if (m_timerReconnect.IsHandle(hSignal))
		{
			if (!m_AgentQ.IsEmpty())
			{
				theLog.Put(DEV_UK, "NGS_Accuse"_COMMA, "[Accuse] timer expired");
				TLock lo(this);
				Accuse();
			}
		}
...
LONG Agent::PostAccusationMsg(AccuseBus & bus, LONG lTimeSpan, RoomID & rid, string *pstrRoomTitle)
{
	...
	if (!AddAccuseNote(dataAR))
	{
		...
		glAC_Dublicate++;
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] Duplicate Accuse : USN ", bus.m_lUSN);
	...
	{
		m_AgentQ.Clear();
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] accusition queue full(100 < size)..so clear queue");
	...
	if(glAC_LogCount > 10)
	{
		...
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "", sTemp.c_str());
...
LONG Agent::Accuse(CAccuseBus_RoomID& dataAR, AccuseBus& bus)
{
	...
	long lASN;
	{
		...
		{
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, " [Accuse] GetSerialNoFromDB ! ");
			return ERRAGENT_SYSTEM;
		}
		if(lASN < 0)
		{
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, " [Accuse] wrong lASN, \"lASN<0\" ! ");
			return ERRAGENT_SYSTEM;
		}
	}
	...
	if(bus.m_sContent.length() > 1)
	{
		...
		if(!SaveChatHistoryToDB(bus, dwASN, pstr))
		{
			...
			glAC_FailTitle++;
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] fail SaveChatHistoryToDB() ");
			return ERRAGENT_SYSTEM;
		}
	...
	{
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, " Content Length() is low.... : length = [", bus.m_sContent.length(), "]");
		return ERRAGENT_SYSTEM;
	}
}
...
BOOL Agent::GetSerialNoFromDB(long &lASN)
{
	...
	if (after > before + 500)
		theLog.Put(WAR_UK,"NGS_DB_Delay, GetSerialNoFromDB(). during:", after - before, ", Query:", strQuery);
...
BOOL Agent::SaveChatHistoryToDB(const AccuseBus &bus, DWORD dwASN, string *pstrRIDAndRoomTitle)
{
	...
	// insert into PL_CASE
	{
		...
		DWORD after = ::GetTickCount();
		if (after > before + 500)
			theLog.Put(WAR_UK,"NGS_DB_Delay, SaveChatHistoryToDB() 1. during:", after - before, ", Query:", strQuery);
		...
		if((bRet == FALSE) || (nDBErrorCode > 0) || (nDecodeError < 0))
		{
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] Agent::SaveChatHistoryToDB() QUERY:", strQuery.c_str(), ", nDecodeError:", nDecodeError);
			return FALSE;
		}
	}
	...
	// insert into PL_PROCESS
	{
		...
		DWORD after = ::GetTickCount();
		if (after > before + 500)
			theLog.Put(WAR_UK,"NGS_DB_Delay, SaveChatHistoryToDB() 2. during:", after - before, ", Query:", strQuery);
...
BOOL Agent::AddAccuseNote (CAccuseBus_RoomID &ARID)
{
	...
	//시간이 오래된거 제거..앞에서 부터 검색
	{
		...
		if(lSize == 500 || lSize == 1000)
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] Accused Item count room title: ", lSize);
...
BOOL HTTPAgent::RunHTTPAgent()
{
	...
	if(m_lHTTPConnFlag != HTTPAGENTSTATE_ACTIVATED)
	{
		theLog.Put(WAR_UK, " NGS_Accuse,HTTPAgent:RunHTTPAgent() ERROR, m_lHTTPConnFlag != HTTPAGENTSTATE_ACTIVATED, m_lHTTPConnFlag:", m_lHTTPConnFlag);
		return FALSE;
	}
	...
	{
		theLog.Put(WAR_UK, "NGS_Accuse, HTTPAgent:RunHTTPAgent() m_lConnFlag != HTTPAGENTCONNSTATE_STOP, m_lConnFlag:", m_lConnFlag);
		return TRUE;
	}
	...
	if(m_apSocket != NULL)
	{
		theLog.Put(WAR_UK, " NGS_Accuse,HTTPAgent:RunHTTPAgent() will return FALSE. Because m_apSocket!= NULL");
		return FALSE;
	}
	...
	{
		delete pLink;
		theLog.Put(WAR_UK, " NGS_Accuse,HTTPAgent:RunHTTPAgent() will return FALSE. Because pLink->Socket()== NULL");
		return FALSE;
	}
	...
	{
		delete pLink;
		theLog.Put(WAR_UK, " NGS_Accuse,HTTPAgent:RunHTTPAgent() called SopHTTPAgent(). Because Attch(pLink) is Failed!");
		return FALSE;
	}
...
BOOL HTTPAgent::ConnectFileServer(HTTPLink *pLink)
{
	...
	{
		theLog.Put(WAR_UK, "NGS_Accuse, HTTPAgent:Connect() m_lConnFlag != HTTPAGENTCONNSTATE_STOP, m_lConnFlag:", m_lConnFlag);
		return FALSE;
	}
	...
	if(!pLink->Connect("boardw.pmang.com", 80))
	{
		if(::WSAGetLastError() != WSAEWOULDBLOCK) 
		{
			...
			theLog.Put(WAR_UK, " NGS_Accuse,HTTPAgent:RunHTTPAgent() called SopHTTPAgent(). Because pLink->Connect is Failed! and WSAGetLastError(",::WSAGetLastError(),") != WSAEWOULDBLOCK");
			return FALSE;
		}
	}
...
BOOL HTTPAgent::OnConnect(TSocket* pSocket, int nErrorCode)
{
	...
	{
		if(!pSocket)
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "+++++ ASSERT : HTTPAgent::OnConnect Error! : Socket NULL");
		else
			theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "+++++ ASSERT : HTTPAgent::OnConnect Error! : [", nErrorCode, "]");
		StopHTTPAgent();
		theLog.Put(WAR_UK, "NGS_Accuse,HTTPAgent:OnConnect() called SopHTTPAgent(). Because (nErrorCode[",nErrorCode,"] || !pSocket)");
		return FALSE;	
	}
...
void HTTPAgent::OnWriteFile()
{
	...
	if(PopAccuseData(m_pairCurAccuseData))
	{
		...
		{
			StopHTTPAgent();
			theLog.Put(WAR_UK, "NGS_Accuse,HTTPAgent:OnWriteFile() called SopHTTPAgent(). Because WriteFile() is Failed. dwASN:",dwASN);
		}
	}
...
BOOL HTTPAgent::WriteFile(DWORD dwASN, CAccuseContent_RoomID& pairContent)
{
	...
	{
		glAC_FailContents++;
		theLog.Put(WAR_UK, "NGS_Accuse, HTTPAgent::WriteFile(). MakeBoundary(",s_bound,") failed");
		return FALSE;
	}
		...
		if(SendHTTPData(s_rData1))
		{
			if(SendHTTPData(s_rData2))
			{
				...
				else
				{
					...
					theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, sLogTemp.c_str());
				}
			}
			else
			{
...
{	
	...
	{
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] TBase::OnReceive(pLink, lErrorCode) \n");
		return FALSE;
	}
...
BOOL HTTPAgent::SendHTTPData(string & sData)
{
	...
	if(!pLink)
	{
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] fail GetSocket from SendHTTPData() \n");
		return FALSE;
	}
...
BOOL HTTPAgent::PushAccuseData(xstring sUserID, xstring sAccusedID, xstring sContent, DWORD dwASN, long nUSN, long lAccusedUSN, RoomID rID, long lTimeSpan)
{
	...
	if(m_vecAccuseDataQ.size() > 100)
	{
		...
		theLog.Put(WAR_UK, "NGS_Accuse"_COMMA, "[Accuse] clear HTTP queue.. because queue full..");
	}
...
BOOL HTTPAgent::ParseHTTP(string& r_buff)
{
	...
	theLog.Put(WAR_UK, " NGS_Accuse"_COMMA, __FUNCTION__, "[Accuse] <<fail cgi call>> ", r_buff.c_str());
